ECMAScript是javascript的核心，但是在web中使用javascript,BOM(Broswer Object Model)浏览器对象模型才是核心。
...
无法精确地做到跨浏览器的条件下取得窗口的左边和上边的精确坐标值。可以使用moveTo(x, y)，x,y代表坐标值。moveBy(leftpx, toppx)。参数代表水平和垂直方向上的移动像素。一般被禁用了。
我自己的实践情况：
